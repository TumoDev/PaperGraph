...
class MenuFrame(tk.Frame):
    ...
    def content(self):
        ...
        title_p, author_p, date_p, content_p =self.controller.display_paper_data()
        ...
        text_box = tk.Text(self, bg="#FFFFFF", fg="#000000", wrap="word", padx=20 , width=10, height=15, font=("Helvetica", 8))
        text_box.insert("end", text)
        text_box.configure(state="disabled")
        scrollbar = tk.Scrollbar(self, command=text_box.yview)
        text_box['yscrollcommand'] = scrollbar.set
        
        text_box.grid(row=5, column=0, columnspan=2, padx=padding_x, pady=padding_y, sticky="nsew")
        scrollbar.grid(row=5, column=1,columnspan=2, padx=padding_x, pady=padding_y,sticky="nse")
        ...
        text_box.grid(row=7, column=0, columnspan=2, padx=padding_x, pady=padding_y, sticky="nsew")
        scrollbar.grid(row=7, column=1,columnspan=2, padx=padding_x, pady=padding_y,sticky="nse")

        # Button download paper
        boton1 = tk.Button(self, text="Download", command=self.on_boton1_click, height=1, width=19)
        boton1.grid(row=8, column=0, padx=(padding_x,4), pady=padding_y, sticky="nsew")
        
        # Button Expand Graph
        boton2 = tk.Button(self, text="Expland", command=self.on_boton2_click,height=1, width=10)
        boton2.grid(row=8, column=1, padx=(4,padding_x), pady=padding_y, sticky="nsew")
